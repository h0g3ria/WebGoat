...
public class JWTVotesEndpoint implements AssignmentEndpoint {
  ...
  public MappingJacksonValue getVotes(
    ...
    if (StringUtils.isEmpty(accessToken)) {
      ...
      try {
        Claims claims = parseAndVerifyToken(accessToken);
  ...
  private Claims parseAndVerifyToken(String accessToken) {
    Jws<Claims> jwt =
    ...
    if (!SignatureAlgorithm.HS512.getValue().equals(jwt.getHeader().getAlgorithm())) {
      throw new UnsupportedJwtException("Only HS512 signed JWTs are accepted");
    }
    return jwt.getBody();
  }
}
